...
class MissionOne(Node):
    def __init__(self):
        ...
        # how to be, what to be stored? not data itself but its metadata communicating with sotrage
        ...
        # Subscriptions
        # data type and which srv has info / which func to use for receiving data 
        ...
        # Publishers
        # data type and whcih srv has info(used for subs to let what has info) / 
    ...
    {'x': 0.0, 'y': 0.0, 'z': 5.0}
        ...
        self.max_s_waypoint = len(self.waypoints)
    ...
    def navigate_waypoint_callback(self):
        # m_target = real target

        
        if self.curr_way_index < self.max_s_waypoint:

            #set the target height to be close to real target height 
            # self.curr_height = self.waypoints[self.curr_way_index]
            ...
            # just started, move to start point. there is no sub waypoint..? also, which should to be first?
          
            if self.is_waypoint_reached(target):
                        self.curr_way_index += 1
                        self.get_logger().info(f"Waypoint {self.curr_way_index} reached")
            else: 
                    # just need to go waypoint, no need to have info about dir?
                ...
                self.position_publisher.publish(pose)
                self.get_logger().info(f"Waypoint {self.curr_way_index} not reached")
                self.get_logger().info(f"current pos is {self.current_position}")
            
                # I think sub waypoint is (x,y), and z will be most likely be cosntant? but why?
                ...
                # this is likely testing swthcing the sattus, vtol after touching 1 and target 2
                # timercount will be to let agetn prepare for vtol..?
                #what if reaches 2 before timercount?
                # if self.timerCount == 200 and self.curr_way_index == 1 and self.vtol_count == 0:
                #     self.arm_vtol(True)
                #     self.vtol_count += 1
                # show the position
                # else : pass
                    # what we need is dir, since we know velocity?
                
                    # if self.curr_way_index == 1:
                    #     self.timerCount += 1
        # if touched all the target routes, land
        else:
