...
class offboard_start(Node):
    ...
    def __init__(self):
        ...
        self.posyaw = 0.0  # Initialize posyaw with a default value
        self.posx = 0.0    # Initialize posx
        self.posy = 0.0    # Initialize posy
        self.posz = 0.0
        ...
        match self.current_state:
            ...
            case "OFFBOARD":
                if(not(self.flightCheck) or self.arm_state == VehicleStatus.ARMING_STATE_ARMED or self.failsafe == True):
                    self.get_logger().info(f"flightcheck->{self.flightCheck},arm_state->{self.arm_state},failsafe->{self.failsafe}")
                    #self.current_state = "IDLE"
